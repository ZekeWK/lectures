...
\begin{problem}
	Hur många bijektiva funktioner finns det mellan två mängder med \(n\) element var?
\end{problem}
...
\begin{problem}
	Vilka potensfunktioner, alltså \(f(x) = x^n\) där \(n\) är ett positivt heltal, är surjektiva? Vilka är injektiva?
\end{problem}
...
\begin{problem}
	Är en injektiv funktion sammansatt med en injektiv funktion injektiv? Är en surjektiv funktion sammansatt med en surjektiv funktion surjektiv?
\end{problem}

\begin{problem}
	Låt \(f(x)\) vara injektiv men inte surjektiv, och \(g(x)\) vara surjektiv men inte injektiv. Vilka av \(h(x) = g(f(x))\) och \(k(x) = f(g(x))\) kan vara inverterbara givet något val av \(f\) och \(g\).
\end{problem}
...
\begin{problem}
	Motivera att \(f(x) = \sum_{k=0} ^{101} (-x)^k\) är surjektiv. (Tips! Använd att den är kontinuerlig)
\end{problem}

\begin{problem}
	Motivera att \(f(x) = \sum_{k=0} ^{100} (-x)^k\) inte är surjektiv. (Tips! Använd att den är kontinuerlig)
\end{problem}
...
\begin{problem}
	...
	\]
	definierad på de \textbf{positiva} reella talen, är inverterbar.
\end{problem}

\begin{problem}
	Hur ser grafen ut för funktion \(f(x) = \begin{cases} 1, & x \text{ rationellt} \\ 0, & x \text{ irrationellt} \end{cases} \) ?
\end{problem}
...
\begin{problem}[Väldigt svår]
	Visa att det inte kan finnas en surjektiv funktion från en mängd till mängden av dess delmängder.
\end{problem}
...
\begin{tips}
	Utnyttja injektivitet! Injektivitet gör att du kan ``skala bort'' funktionen, alltså att \(f(x) = f(y)\) ger \(x = y\).
\end{tips}
...
\begin{obs}
	\(\mathbb{R} \) är mängden av alla reella tal, $\mathbb{Q}$ är mängden av alla rationella tal (tal som kan skrivas som en kvot av två heltal), och \(\mathbb{Z} \) är mängden av alla heltal. Om \(A, B\) är två mängder så kan vi skriva \(f: A \rightarrow B\), att \(f\) är en funktion från \(A\) till \(B\).
\end{obs}
...
\setcounter{theorem}{0}
\begin{problem}
	Bestäm \(f : \mathbb{R} \rightarrow \mathbb{R} \) om
	\[
		f(x) + f(y) = 2.
	\]
\end{problem}
...
\begin{problem}
	Beskriv alla funktioner \(f: \mathbb{R} \rightarrow \mathbb{R}, g : \mathbb{R} \rightarrow \mathbb{R}  \) sådana att
	\[
		f(x) \cdot g(x) = 0.
	\]
\end{problem}
...
\begin{problem}
	Låt \(f: \mathbb{Q} \rightarrow \mathbb{Q} \) vara en funktion som uppfyller att
	\[
		f(x) \cdot f(y)=f(x+y)
	\]
	och \(f(0) =2\). Hitta alla möjliga \(f\).
\end{problem}
